...
def test_at1_10_dynamic_spam_profile_lifecycle(application_server) -> None:
    """AT1.10 dynamic SPAM profile lifecycle over real API and MCP transports.
    ...
    with httpx.Client(base_url=application_server.api_base_url, timeout=90.0) as api_client:
        ...
        try:
            ...
            with httpx.Client(
                ...
                print(
                    "[AT1.10] profile=%s folder=%s total=%d sample_subjects=%s"
                    % (profile_id, spam_folder, len(messages), sample_subjects)
                )
        finally:
            if created:
