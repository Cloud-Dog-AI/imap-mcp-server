...
def _config(tmp_path: Path) -> GlobalConfigModel:
    return GlobalConfigModel(
        server=ServerConfig(
            server_id="imap-it-timeout",
            auth=ServerAuthConfig(mode="api_key"),
            audit=AuditConfig(log_path=str(tmp_path / "timeout-audit.jsonl")),
...
def test_it122_job_recovery_after_timeout_requeues_and_completes(tmp_path: Path) -> None:
    runtime = build_jobs_runtime(_config(tmp_path), worker_suffix="timeout")
    attempts = {"count": 0}

    def _handler(_: JobEnvelope) -> dict[str, str]:
        attempts["count"] += 1
        if attempts["count"] == 1:
    ...
    try:
        ...
        job_id = runtime.submit(JobEnvelope(job_type="sync", profile_id="ops", payload={}))

        assert runtime.run_once() is True
        first = runtime.get_state(job_id)
        assert first is not None
        assert first.status == "queued"
        assert first.attempts == 1
        assert first.last_error == "simulated timeout"

        assert runtime.run_once() is True
        second = runtime.get_state(job_id)
        assert second is not None
        assert second.status == "succeeded"
        assert second.attempts == 2
        assert runtime.last_result(job_id) == {"ok": "done"}
    finally:
        runtime.close()
