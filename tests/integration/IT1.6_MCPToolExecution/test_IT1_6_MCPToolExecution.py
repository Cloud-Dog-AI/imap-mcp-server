...
def test_it16_mcp_tool_execution(integration_server) -> None:
    # W28A-735-R5: the MCP transport now requires a valid API key (anon -> 401);
    # tool execution is served only to an authenticated caller.
    with httpx.Client(base_url=integration_server.mcp_base_url, timeout=30.0) as client:
        response = client.post(
            integration_server.mcp_path("/tools/profile_list"),
