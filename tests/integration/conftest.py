...
def integration_server(request: pytest.FixtureRequest) -> Iterator[IntegrationServer]:
    ...
    if isinstance(env_values, str):
        ...
        env_items = []
    env_files = [Path(item) for item in env_items] if env_items else [ROOT / "tests" / "env-IT"]
    missing = [path for path in env_files if not path.exists()]
    if missing:
        pytest.fail(f"Missing integration env file(s): {', '.join(str(path) for path in missing)}")
    ...
    try:
        ...
        if start.returncode != 0:
            pytest.fail(
                f"Failed to start integration services:\nstdout:\n{start.stdout}\nstderr:\n{start.stderr}"
            )
        started = True
