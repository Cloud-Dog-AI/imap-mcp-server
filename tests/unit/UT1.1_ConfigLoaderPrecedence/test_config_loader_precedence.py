# Copyright 2026 Cloud-Dog, Viewdeck Engineering Limited
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
#     http://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.

from __future__ import annotations

# Covers: FR-02

from types import SimpleNamespace

from imap_hub_core.config import loader
from tests.helpers.ports import listener_host, listener_port
import pytest
@pytest.mark.UT
@pytest.mark.mcp
@pytest.mark.req("FR-02")


def test_load_global_config_delegates_to_cloud_dog_loader(monkeypatch) -> None:
    captured: dict[str, object] = {}

    def fake_load_config(*, env_files, config_yaml, defaults_yaml):
        captured["env_files"] = env_files
        captured["config_yaml"] = config_yaml
        captured["defaults_yaml"] = defaults_yaml
        return SimpleNamespace(
            data={
                "server": {
                    "auth": {"mode": "api_key"},
                    "audit": {"log_path": "./data/audit/audit.jsonl"},
                    "storage": {
                        "data_dir": "./data",
                        "downloads_dir": "./data/downloads",
                        "archive_dir": "./data/archive",
                    },
                    "limits": {
                        "max_search_results": 100,
                        "max_message_bytes": 1000,
                        "max_attachment_bytes": 1000,
                        "extractor_timeout_sec": 10,
                    },
                },
                "api_server": {
                    "host": listener_host("CLOUD_DOG__API_SERVER__HOST"),
                    "port": listener_port("CLOUD_DOG__API_SERVER__PORT"),
                },
                "web_server": {
                    "host": listener_host("CLOUD_DOG__WEB_SERVER__HOST"),
                    "port": listener_port("CLOUD_DOG__WEB_SERVER__PORT"),
                },
                "mcp_server": {
                    "host": listener_host("CLOUD_DOG__MCP_SERVER__HOST"),
                    "port": listener_port("CLOUD_DOG__MCP_SERVER__PORT"),
                    "transport": "streamable-http",
                },
                "a2a_server": {
                    "host": listener_host("CLOUD_DOG__A2A_SERVER__HOST"),
                    "port": listener_port("CLOUD_DOG__A2A_SERVER__PORT"),
                },
                "sync": {"schedule_sec": 60},
                "index": {"schedule_sec": 60},
                "rbac": {"enabled": True, "roles": {}},
                "profiles": {},
            }
        )

    monkeypatch.setattr(loader, "load_config", fake_load_config)
    model = loader.load_global_config(env_files=["tests/env-UT"])

    assert model.api_server.host == "127.0.0.1"
    assert captured["env_files"] == ["tests/env-UT"]
    assert captured["config_yaml"] == "config.yaml"
    assert captured["defaults_yaml"] == "defaults.yaml"
